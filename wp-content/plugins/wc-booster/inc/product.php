...
if( !class_exists( 'WC_Booster_Product' ) ){
	class WC_Booster_Product{
		...
		public function init(){
			...
			add_action( 'woocommerce_after_add_to_cart_form', array( $this, 'usp' ) );
		...
		public function usp(){
			...
			global $product;
			$usps = get_post_meta( $product->get_id(), 'wc_booster_product_usps', true );
			if( is_array( $usps ) ){
				?>
				<ul class="wc-booster-usps">
				<?php
					foreach( $usps as $usp ){
						foreach( $usp as $u ){
							?>
							<li><?php echo esc_html( $u[ 'value' ] ); ?></li>
							<?php
						}
					}
				?>
